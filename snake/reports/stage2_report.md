...
本阶段完成了手写 DQN、经验回放、目标网络、探索日程、独立验证、模型演示与完整训练恢复，并运行训练种子 0 的 300000 个环境步。初始网络验证平均食物数为 0，验证最优模型在第 **260000** 步达到 **1.30**，末次为 **1.05**。与第一阶段启发式的 17.25 相比仍有明显差距。
...
本次累计 74751 次梯度更新，完成 5833 个回合，预算末尾另有一个 19 步未结束回合，已保存以便恢复。训练循环用时约 79.74 秒；包含初始/周期验证、存档和日志等的运行段墙钟约 104.33 秒。该墙钟起点在 Trainer 创建之后，不包含 Python 导入和网络初次构造。Windows 进程峰值工作集约 1062.2 MiB，包括运行、存档及框架内存；本次训练不在 GPU 上更新参数。
...
| DQN 初始网络 | 0 | 0.00 | 0.00 | 2000.0 | 0/20 | 20/20 |
| DQN 验证最优 | 260000 | 1.30 | 1.13 | 822.4 | 12/20 | 8/20 |
| DQN 末次网络 | 300000 | 1.05 | 1.10 | 1217.4 | 8/20 | 12/20 |
...
运行来源哈希：`96141f7d9ebea738c41b88a1065e95cc0310f2fcd3bfa91f656c30383924a182`。最优模型 SHA-256：`d0dbe6b3811ee2da0c7e4387985f1487edc9c872ad1ace2a2db32adafa0056f9`。其他文件的校验值见归档 summary.json。
